    ...
    <div @click="getFamousQuotes" :class="['hero', { 'hide-banner': !showBanner }]">
      <div class="home-blog-hero">
        <ModuleTransition delay=".06">
          <img v-if="recoShowModule && $frontmatter.heroImage" class="hero-img" alt="wudeh的学习基地" :src="$withBase($frontmatter.heroImage)" />
        </ModuleTransition>
...
export default {
  ...
  computed: {
    ...
    // 没有分页参数 p 隐藏banner栏
    showBanner() {
      return this.$route.query.p && (!this.homeData.postList || this.homeData.postList === "detailed") ? false : true;
    },
  },
  created() {
